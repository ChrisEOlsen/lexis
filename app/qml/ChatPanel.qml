...
// while the model is still answering the first (see AppController.h on
// why that is a hard constraint, not a UX nicety).
//
// Answers stream live: QueryWorker's queryToken arrives piece by piece
// and grows the newest message row (model.isLive -- see
// ChatMessageListModel.h), rendered through markdownSafePrefix so the
// partial text never paints raw markup. The footer shows the pipeline's
// real stage text (AppController.queryStageText) while the answer is
// being produced.
//
...
Item {
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            // Settings. Always enabled -- settings are useful before any
            // group exists.
            Button {
                id: settingsButton
                flat: true
                // Square, like the sidebar toggle at the top of this row:
                // the style's default Button padding is sized for a word,
                // and around a single glyph it reads as a wide empty box.
                // Horizontal padding only -- the vertical padding is what
                // keeps this the same height as History/New chat.
                leftPadding: 0
                rightPadding: 0
                implicitWidth: 38
        ...
        ListView {
            ...
            delegate: Item {
                ...
                // Row position, for the "Try harder" button below: the
                // retry always acts on the conversation's newest answer,
                // so only the newest answer may offer it.
                required property int index
                width: messageList.width
                height: messageColumn.height

                // Live streaming: the answer's text grows by token while
                // isLive is true (see ChatMessageListModel.h). The partial
                // prefix is rendered through markdownSafePrefix so a
                // half-arrived "**bold**" or link never paints its raw
                // markup; the finished answer renders verbatim. History
                // rows are never live and render instantly, unchanged.
                ...
                Column {
                    ...
                    Label {
                        ...
                        // Once the answer is finished, show the stored text
                        ...
                        text: messageDelegate.live
                              ? root.markdownSafePrefix(messageDelegate.model.text)
                    ...
                    // Answer actions, shown once the answer is complete:
                    // sources (when something was retrieved), copy, a
                    // deeper-search retry (SEARCH answers only), and
                    // per-answer feedback. A row of small flat buttons, not
                    // icons -- text labels keep each action's meaning
                    // discoverable without hovering anything.
                    Row {
                        ...
                        Button {
                            flat: true
                            // Shown only when something was actually
                            // retrieved. An answer with no sources -- the
                            // CHAT path, or a search that matched nothing --
                            // gets no affordance at all rather than a "no
                            // source" label: a control whose only message is
                            // that it has nothing to show is noise on every
                        ...
                        Button {
                            flat: true
                            // "Try harder" re-runs the retrieval with the
                            // score floor off and the reasoning pass on.
                            // Offered only on SEARCH answers -- a CHAT reply
                            // has nothing retrieved to deepen, and a SUMMARY
                            // answer is about the overview, not passages --
                            // and only on the LAST row, because that is the
                            // answer retryLastAnswer() replaces: on any
                            // older row the click would silently regenerate
                            // a different message. canRetryLastAnswer covers
                            // the rest (no query running, the question still
                            // known, the group answerable), so the button is
                            // never shown when clicking it would do nothing.
                            visible: messageDelegate.model.tool === "search"
                                     && messageDelegate.index === messageList.count - 1
            ...
            footer: Item {
                ...
                Row {
                    ...
                    Label {
                        // The live stage ("Searching the group...",
                        // "Reading 12 passages...", "Writing...") from
                        // AppController -- replaced the old static
                        // "Thinking...", which sat unchanged for the
                        // entire 8-20s wait and read as a hang. Empty
                        // text falls back to the placeholder below so
                        // a stage that never reported never blanks the
                        // footer mid-query.
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        ColumnLayout {
                            ...
                            // Identity line + the one action every real
                            // passage card exists for: open the document
                            // at this chunk (see DocumentViewerDialog).
                            // chunkId is only present on SEARCH sources
                            // (a specific matched passage); the
                            // SUMMARY path's whole-document rows have no
                            // chunk to focus -- open at the top.
                            RowLayout {
                                spacing: Theme.spacingXS
                                Layout.fillWidth: true
                                ...
                                Button {
                                    ...
                                    font.pixelSize: Theme.fontSizeCaption
                                    text: qsTr("Open")
                                    onClicked: documentViewer.openFor(sourceCard.modelData.documentName,
